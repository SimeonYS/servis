...
class ServisPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `servis`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM servis WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO servis (date, title, link, content)"
